...
write(paste0("Barcode2cluster info is from Sudhir (see email 12/08/2020)\n"),file=file.rmd.name,append=TRUE)
...
metadata.raw <- read.delim(file = paste0(path.raw.data,
                                         "BarcodesMatchingClusterFullAtlasNewClusterNames.txt"),
                           stringsAsFactors = F)
metadata.raw<-separate(metadata.raw, col = names(metadata.raw)[1],
                      into = c("Sample", "Barcode"), remove = F, sep = "_")

unique(metadata.raw$Sample)
metadata.raw$Group<-gsub(metadata.raw$Sample, 
                         pattern = "\\d", 
                         replacement = "")
...
names(metadata)
file.name<-unique(metadata.raw$Sample)
file.name
...
for (i in 1:8) {
  ...
  names(tmp)[2:ncol(tmp)]<-paste0(file.name[i], "_", names(tmp)[2:ncol(tmp)])
  tmp.mtx[[i]]<-tmp
}
...
SCSeq.mtx<-list()
## cherry
SCSeq.mtx[[1]]<-tmp.mtx[5:8] %>% reduce(merge, by = "GENE")
## Notum
SCSeq.mtx[[2]]<-tmp.mtx[1:4] %>% reduce(merge, by = "GENE")
## merged
SCSeq.mtx[[3]]<-tmp.mtx %>% reduce(merge, by = "GENE")
...
for (i in 1:length(SCSeq.mtx)) {
  ...
  SCSeq.mtx[[i]] <- SCSeq.mtx[[i]][ , metadata[[i]]$Cell.ID.Barcodes]
  # SCSeq.mtx[[i]] %>% str() %>% print()
  all.equal(metadata[[i]]$Cell.ID.Barcodes, colnames(SCSeq.mtx[[i]])) %>% print()
}
...
for (i in 1:length(SCSeq.mtx)) {
  ...
  str(metadata[[i]])
  cell2clusterAssignment<-metadata[[i]][,c(1,4)]
