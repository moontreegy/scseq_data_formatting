...
###read matrix
library(Matrix)
SCSeq.mtx<- readMM(paste0(path.raw.data,"GSE104556_matrix.mtx"))
SCSeq.colnames.barcode<- read.delim(paste0(path.raw.data,"GSE104556_barcodes.tsv"),
                                    stringsAsFactors = F,header = F)
SCSeq.rownames.gene<- read.delim(paste0(path.raw.data,"GSE104556_genes.tsv"),
                                 stringsAsFactors = F,header = F)

dim(SCSeq.mtx)
dim(SCSeq.colnames.barcode)
dim(SCSeq.rownames.gene)
...
# metadata
metadata <- readxl::read_excel(paste0(path.raw.data, "SuppTableCells.xlsx"))
str(metadata)
metadata<-metadata[,c(1,9)]

unique(metadata$Barcode) %>% length()
unique(metadata$CellType)

intersect(metadata$Barcode, SCSeq.colnames.barcode$V1)
