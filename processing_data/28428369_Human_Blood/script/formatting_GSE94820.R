...
write(paste0("Note: data is acquired from: https://singlecell.broadinstitute.org/single_cell/bulk_data/SCP43/atlas-of-human-blood-dendritic-cells-and-monocytes/all/621118\n"),file=file.rmd.name,append=TRUE)


# curl "https://singlecell.broadinstitute.org/single_cell/bulk_data/SCP43/atlas-of-human-blood-dendritic-cells-and-monocytes/all/621118" -o cfg.txt; curl -K cfg.txt; rm cfg.txt

list.files(path=path.raw.data,recursive = TRUE,full.names = T)

### read a logTransformed matrix
SCSeq.mtx<- read.delim(paste0(path.raw.data,
                                   "expression_matrix_tpm.txt"),
                            stringsAsFactors = F)
...
# seuratObj <- NormalizeData(seuratObj)  ### not run for logtransformed data
